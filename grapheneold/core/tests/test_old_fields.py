from grapheneold.libraries.graphql.type import (GraphQLBoolean, GraphQLField, GraphQLFloat,
                          GraphQLID, GraphQLInt, GraphQLNonNull, GraphQLString)
from py.test import raises

from grapheneold.core.fields import (BooleanField, Field, FloatField, IDField,
...
def test_field_repr():
    f = StringField().as_field()
    assert repr(f) == "<grapheneold.core.types.field.Field>"


def test_field_repr_contributed():
    f = StringField().as_field()
    f.contribute_to_class(MyOt, 'field_name')
    assert repr(f) == "<grapheneold.core.types.field.Field: field_name>"
