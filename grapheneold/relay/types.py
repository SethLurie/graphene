...
from grapheneold.libraries.graphql_relay.connection.arrayconnection import connection_from_list
from grapheneold.libraries.graphql_relay.node.node import to_global_id
...
class Edge(ObjectType):
    ...
    def for_node(cls, node):
        from grapheneold.relay.utils import is_node
    ...
    def construct_get_node(cls):
        ...
        if get_node_num_args < args:
            warnings.warn("get_node will receive also the info arg"
                          " in future versions of grapheneold".format(cls.__name__),
                          FutureWarning)
